  ...
  if (notFound || !property) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <Link
          to="/listings"
          className="mt-6 inline-block rounded-xl bg-navy px-6 py-3 font-semibold text-beige"
        >
          Back to listings
        </Link>
      </div>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <nav className="mb-4 text-sm text-slate/60">
        ...
        <Link to="/listings" className="hover:text-navy">
          Listings
        </Link>
        <span className="mx-2">/</span>
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="overflow-hidden rounded-2xl bg-slate/10 ring-1 ring-navy/5">
            {cover ? (
              ...
              <div className="flex aspect-[16/10] items-center justify-center text-slate/40">
                No image
              </div>
            )}
          </div>
          ...
          {property.video && (
            <div className="mt-6">
              ...
              <video
                src={property.video}
                controls
                className="w-full rounded-2xl ring-1 ring-navy/10"
              />
            </div>
          )}
        ...
        <aside className="lg:col-span-1">
          <div className="sticky top-24 rounded-2xl bg-white p-6 shadow-card ring-1 ring-navy/5">
            ...
            {property.address && (
              <p className="mt-1 text-sm text-slate/60">{property.address}</p>
            )}

            <p className="mt-4 font-serif text-3xl font-bold text-gold-dark">
