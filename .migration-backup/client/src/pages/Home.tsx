    ...
    <div>
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-24 sm:py-32">
          <span className="inline-block rounded-full bg-gold/20 px-4 py-1.5 text-sm font-medium text-gold-light ring-1 ring-gold/30">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              to="/listings"
              className="rounded-xl bg-gold px-6 py-3 font-semibold text-navy transition hover:bg-gold-light"
            >
              Browse Properties
            </Link>
            <Link
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="mb-8 flex items-end justify-between">
          ...
          <Link to="/listings" className="hidden text-sm font-semibold text-gold-dark hover:underline sm:block">
            View all →
          </Link>
        </div>
      ...
      <section className="bg-navy py-16">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 sm:grid-cols-3">
          {[
            { title: "Mapped Coverage", body: "Every listing shows its exact area drawn on an interactive map." },
            { title: "Near You", body: "Allow location access to instantly see properties around you." },
            { title: "Direct Contact", body: "Reach our team on WhatsApp in one tap, no forms lost in inboxes." },
          ].map((f) => (
            <div key={f.title} className="rounded-2xl bg-white/5 p-6 ring-1 ring-white/10">
              <h3 className="font-serif text-xl font-semibold text-gold-light">{f.title}</h3>
